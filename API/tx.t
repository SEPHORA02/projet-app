            ...
            if sensor_data:
                ...
                if is_anomaly:
                    ...
                    if analysis and analysis.get("should_alert"):
                        logger.critical(f"🚨 ALERTE ASTHME: {analysis.get('message')}")
                        
                        alert_sent = await dashboard_service.send_alert(sensor_data, analysis)
                        
                        if alert_sent:
                            logger.info("✅ Alerte envoyée au dashboard")
                        else:
                            logger.error("❌ Échec envoi alerte")
                    else:
                        logger.info("ℹ️ Anomalie détectée mais pas de risque critique")
                else:
