...
def inscription(request):
    ...
    return render(request, 'health/register.html', {"donnees": donnees, "erreurs": erreurs})
...
def tableau_de_bord(request):
    ...
    try:
        donnees_api = recuperer_donnees_fastapi(patient, id_token="")  # Ajouter id_token réel ici
        analyse_risque = analyser_risque_asthme(donnees_api)
        if analyse_risque["risque"]:
            recents = Alert.objects.filter(
                patient=patient, 
            ...
            if not recents:
                Alert.objects.create(
                    patient=patient,
                    title="Risque de crise d'asthme détecté",
                    message="Les données externes indiquent un risque potentiel: " + "; ".join(analyse_risque["facteurs"]),
                    alert_type='warning',
                    level='high',
                )
    except ErreurRecuperationDonnees as exc:
